(function () {
  ...
  function currentConversationHash() {
    if (!window.location.hash.startsWith("#conversation-")) return "";
    return window.location.hash.replace("#conversation-", "").trim();
  }

  function setConversationHash(conversationId) {
    const nextHash = `#conversation-${conversationId}`;
  ...
  function registerServiceWorker() {
    ...
    navigator.serviceWorker.register("admin-sw.js?v=20260619c").catch(() => {});
  }
  ...
  function cacheNodes() {
    ...
    state.nodes.threadDeleteButton = node("threadDeleteButton");
  ...
  function renderConversations() {
    ...
    state.conversations.forEach((conversation) => {
      ...
      const needsReply = Number(conversation.unread_count || 0) > 0;
      ...
      const unreadMarkup = needsReply
        ? `<span class="admin-conversation-badge unread">${conversation.unread_count}</span>`
        : "";

      button.innerHTML = `
        <div class="admin-conversation-top">
          <h3 class="admin-conversation-name">${conversation.visitor_name || "Website Visitor"}</h3>
          <div class="admin-conversation-badges">
            ${unreadMarkup}
            <span class="admin-conversation-badge">${formatStatusLabel(conversation.status)}</span>
          </div>
        </div>
  ...
  async function markConversationRead(conversationId) {
    ...
    if (existingConversation && Number(existingConversation.unread_count || 0) === 0) return;
    ...
    if (error) {
      console.error("Hospera admin could not mark conversation as read.", error);
      return;
    }
  ...
  async function loadConversations() {
    ...
    const hashConversationId = currentConversationHash();
    if (hashConversationId && (!state.currentConversation || state.currentConversation.id !== hashConversationId)) {
      const fromHash = state.conversations.find((item) => item.id === hashConversationId);
      if (fromHash) {
        await selectConversation(fromHash.id);
        return;
      }
    }
  ...
  async function selectConversation(conversationId) {
    ...
    const refreshed = state.conversations.find((item) => item.id === conversation.id) || conversation;
    state.currentConversation = refreshed;
    renderConversations();
    renderCurrentConversation();
    setConversationHash(conversation.id);
    setThreadStatus("");
  }
  ...
  async function handleReply(event) {
    ...
    const nextStatus = state.currentConversation.status === "closed" ? "in_progress" : state.currentConversation.status || "in_progress";
    const readAt = new Date().toISOString();
  ...
  async function updateConversationStatus() {
    ...
    state.conversations = state.conversations.map((conversation) =>
      conversation.id === state.currentConversation.id ? { ...conversation, status: nextStatus } : conversation
    );
    renderConversations();
    renderCurrentConversation();
  ...
  async function deleteConversation() {
    ...
    const confirmed = window.confirm("Delete this full chat permanently? This removes all messages for this visitor conversation.");
    if (!confirmed) return;

    setThreadStatus("Deleting chat...");
    const conversationId = state.currentConversation.id;
    const { error } = await state.supabase.from("chat_conversations").delete().eq("id", conversationId);

    if (error) {
      setThreadStatus(error.message || "Chat could not be deleted.", "error");
      return;
    }
  ...
  function bindAppEvents() {
    ...
    state.nodes.threadDeleteButton.addEventListener("click", deleteConversation);
    window.addEventListener("hashchange", async () => {
      const conversationId = currentConversationHash();
      if (!conversationId) return;
      if (state.session) {
