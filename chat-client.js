(function () {
  ...
  function syncConversationToTracker(profile, openingMessage) {
    ...
    try {
      ...
      console.warn("Hospera live chat tracker sync failed.", error);
    }
  }
  ...
  async function createConversation(profile, openingMessage) {
    ...
    try {
      ...
      console.warn("Hospera live chat started, but the thread UI could not fully hydrate.", hydrateError);
      state.messages = [
        {
          id: `local-${Date.now()}`,
          sender_type: "visitor",
  ...
  function bindEvents() {
    ...
    state.nodes.prechat.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        console.error("Hospera live chat conversation could not start.", error);
